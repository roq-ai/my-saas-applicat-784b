...
export const getCustomerInquiries = async (query?: CustomerInquiryGetQueryInterface) => {
  const response = await axios.get(`/api/customer-inquiries${query ? `?${queryString.stringify(query)}` : ''}`);
